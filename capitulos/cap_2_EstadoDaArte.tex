...
\footnotetext{Preferred Reporting Items for Systematic Reviews and Meta-Analyses 2020 Statement.}

\begin{figure}[H]
    \centering
    \begin{tikzpicture}[
        node distance=1.1cm and 2.2cm,
        box/.style={rectangle, draw, fill=blue!15, text width=6.5cm, minimum height=1.2cm, align=center, font=\small},
        excl/.style={rectangle, draw, fill=gray!15, text width=4.5cm, minimum height=1.2cm, align=left, font=\footnotesize},
        arrow/.style={-Stealth, thick, blue!80!black}
    ]
        % --- FASE DE IDENTIFICAÇÃO ---
        \node (ident) [box] {\textbf{Identificação}\\Registos identificados em bases de dados científica\\(IEEE Xplore, ACM Digital Library e Scopus)\\($N = 150$)};
        
        \node (dedup) [excl, right=of ident] {Registos removidos antes da triagem:\\$\bullet$ Duplicados ($n=25$)\\$\bullet$ Irrelevância direta ($n=5$)\\($N = 30$)};
        \draw [arrow] (ident) -- (dedup);

        % --- FASE DE TRIAGEM ---
        \node (screen) [box, below=1.2cm of ident] {\textbf{Triagem}\\Registos triados (Título e Resumo)\\($N = 120$)};
        \draw [arrow] (ident) -- (screen);

        \node (excl_screen) [excl, right=of screen] {Registos excluídos:\\$\bullet$ Títulos fora do âmbito ($n=45$)\\$\bullet$ Resumos não técnicos ($n=30$)\\($N = 75$)};
        \draw [arrow] (screen) -- (excl_screen);

        % --- FASE DE ELEGIBILIDADE ---
        \node (elig) [box, below=1.2cm of screen] {\textbf{Elegibilidade}\\Artigos avaliados na íntegra para elegibilidade\\($N = 45$)};
        \draw [arrow] (screen) -- (elig);

        \node (excl_elig) [excl, right=of elig] {Artigos excluídos:\\
            $\bullet$ Ausência de prova formal ($n=8$)\\
            $\bullet$ Protocolo não auditável ($n=7$)\\
            $\bullet$ Fora do âmbito eleitoral ($n=5$)\\
            ($N = 20$)};
        \draw [arrow] (elig) -- (excl_elig);

        % --- FASE DE INCLUSÃO ---
        \node (incl) [box, below=1.4cm of elig, fill=blue!25, thick] {\textbf{Inclusão}\\Estudos selecionados para análise comparativa\\no Estado da Arte\\($N = 25$)};
        \draw [arrow] (elig) -- (incl);

    \end{tikzpicture}
    \caption{Fluxograma PRISMA 2020 do processo de revisão da literatura. Adaptado das diretrizes PRISMA 2020 para o contexto de sistemas de votação eletrónica baseados em blockchain.}
    \label{fig:prisma}
\end{figure}
