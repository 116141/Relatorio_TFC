...
\begin{enumerate}
    \item \textbf{Camada de Dados (Blockchain)} – rede pública Ethereum Sepolia onde residem os smart contracts responsáveis pelo registro de eleitores, submissão de votos e apuração.
    ...
    \item \textbf{Camada de Aplicação (Frontend)} – interface web desenvolvida em React + Ethers.js, projetada segundo WCAG 2.1 (nível AA) para garantir acessibilidade.
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{RegistroEleitor.sol}\ – gerencia o cadastro de identidades digitais (endereço Ethereum ↔ identidade institucional). Utiliza um mapeamento `address => bytes32` e verifica a unicidade via `require(!registered[msg.sender])`.
...
\begin{enumerate}
    \item Implementar `RegistroEleitor.sol` e `VotoAnonimo.sol` com suporte a ZK‑SNARKs.
